...
#define CONNECTION_TIME_ADDRESS 2040
public:
  P_Memory() {}
  ...
  void get_device_id(char *id);
  void set_device_id(char *id);
  unsigned int get_connection_time();
  void set_connection_time(unsigned int time);
...
void setup()
{
  ...
  // for(int i=1; i<32;i++)
  // {
  // sprintf(buf,"%s/Schedule/Dec/%d",device_id,i);
...
static void check_slave_request(void)
{
  static unsigned int update_ref=millis();
  if (check_serial2())
  {
  ...
  {
     if((unsigned int)(millis()-update_ref)>connection_time*1000)
     {
        update_ref=millis();
        Serial2.print("{\"topic\":\"time\",\"value\":\"?\"}");
        delay(100);
        if (check_serial2())
        {
          DeserializationError error = deserializeJson(Json.doc, uart2.buffer);
          ...
          else
          {
            String topic = Json.doc["topic"];
            String value = Json.doc["value"];
            if(topic=="time")Firebase.RTDB.setString(&fbdo, root_path + "Last_Connected", value);
...
static void check_firebase_request(void)
{
  if (Firebase.ready() && signupOK)
  {
    ...
    {
      if (fbdo_stream.boolData() == true)
      {
        Firebase.RTDB.setBool(&fbdo, root_path + "Update", false);
        FirebaseJson json;
        ...
        char buffer[200];
        char serial[200];
        if (Firebase.RTDB.getJSON(&fbdo, root_path + "Server_Request"))
          json = fbdo.jsonObject();
        if (json.get(result, "topic"))
          topic = result.stringValue;
        ...
        if (topic == "url")
        {
          if (value == "?")
          {
            ...
            if (json.setJsonData(serial))
              Firebase.RTDB.setJSON(&fbdo, root_path + "Device_Response", &json);
          }
          else
          {
            if (value != NULL)
        ...
        else if (topic == "schedule")
        {
          ...
          if (value == "Year")
          {
            ...
            for (int i = 0; i < 12; i++)
            {
              ...
              {
              case 0:
                strcat(month, "/Schedule/Jan/");
                break;
              case 1:
                strcat(month, "/Schedule/Feb/");
                break;
              case 2:
                strcat(month, "/Schedule/Mar/");
                break;
              case 3:
                strcat(month, "/Schedule/Apr/");
                break;
              case 4:
                strcat(month, "/Schedule/May/");
                break;
              case 5:
                strcat(month, "/Schedule/Jun/");
                break;
              case 6:
                strcat(month, "/Schedule/Jul/");
                break;
              case 7:
                strcat(month, "/Schedule/Aug/");
                break;
              case 8:
                strcat(month, "/Schedule/Sep/");
                break;
              case 9:
                strcat(month, "/Schedule/Oct/");
                break;
              case 10:
                strcat(month, "/Schedule/Nov/");
                break;
              case 11:
                strcat(month, "/Schedule/Dec/");
                break;
              }
          ...
          else if (check_day(value, &m, &d))
          {
            ...
            if (Firebase.RTDB.getString(&fbdo, buf))
            {
              ...
              if (parse_schedule(val, m, d, &sc))
              {
                ...
                {
                  Serial.println(uart2.buffer);
                  if (json.setJsonData(uart2.buffer))
                    Firebase.RTDB.setJSON(&fbdo, root_path + "Device_Response", &json);
                }
              }
            }
        ...
        else if (topic == "device_id")
        {
          if (value == "?")
          {
            sprintf(serial, "{\"topic\":\"device_id\",\"value\":\"%s\"}", device_id);
            if (json.setJsonData(serial))
              Firebase.RTDB.setJSON(&fbdo, root_path + "Device_Response", &json);
          }
          else
          {
            if (value != NULL)
            {
              int len = value.length() + 1;
              value.toCharArray(buffer, len);
              ESP32_Flash.set_device_id(buffer);
              sprintf(serial, "{\"topic\":\"status\",\"value\":\"ok\"}");
              if (json.setJsonData(serial))
                Firebase.RTDB.setJSON(&fbdo, root_path + "Device_Response", &json);
            }
        ...
        else if(topic=="connection")
      {
        if (value == "?")
        {
          sprintf(serial, "{\"topic\":\"connection\",\"value\":\"%d\"}", connection_time);
          if (json.setJsonData(serial))
            Firebase.RTDB.setJSON(&fbdo, root_path + "Device_Response", &json);
        }
        else
        {
          if (value != NULL)
          {
            int len = value.length() + 1;
            value.toCharArray(buffer, len);
            ESP32_Flash.set_connection_time(atoi(buffer));
            connection_time=atoi(buffer);
            sprintf(serial, "{\"topic\":\"status\",\"value\":\"ok\"}");
            if (json.setJsonData(serial))
              Firebase.RTDB.setJSON(&fbdo, root_path + "Device_Response", &json);
          }
...
static bool check_query(String value, uint8_t *month, uint8_t *day)
{
  ...
  *day = value.substring(start, len).toInt();
  if (*day > Number_Of_Days_In_Months[*month - 1] && *day < 1)
    return false;
  return true;
}
  ...
  if (check_serial())
  {
    ...
    else
    {
      ...
      else if(topic=="connection")
      {
        if (value == "?")
        {
          Serial.print("{\"topic\":\"connection\",\"value\":\"");
          Serial.printf("%d", connection_time);
          Serial.println("\"}");
        }
        else
        {
...
static void setup_network(void)
{
  ...
  ESP32_Flash.get_device_id(device_id);
  connection_time=ESP32_Flash.get_connection_time();
  ...
  root_path += "/";
  if (signupOK)
    if (!Firebase.RTDB.beginStream(&fbdo_stream, root_path + "Update"))
...
void P_Memory::storeSchedule(uint8_t month, uint8_t day, schedule sc)
{
  if (month > 12 || month == 0)
    return;
  if (day > Number_Of_Days_In_Months[month - 1] || day == 0)
    return;
...
void P_Memory::get_device_id(char *id)
{
  id[0] = EEPROM.read(CONTROLLER_ID_ADDRESS);
  id[1] = EEPROM.read(CONTROLLER_ID_ADDRESS + 1);
  id[2] = EEPROM.read(CONTROLLER_ID_ADDRESS + 2);
  id[3] = EEPROM.read(CONTROLLER_ID_ADDRESS + 3);
}
void P_Memory::set_device_id(char *id)
{
  EEPROM.write(CONTROLLER_ID_ADDRESS, id[0]);
  EEPROM.write(CONTROLLER_ID_ADDRESS + 1, id[1]);
  EEPROM.write(CONTROLLER_ID_ADDRESS + 2, id[2]);
  EEPROM.write(CONTROLLER_ID_ADDRESS + 3, id[3]);
  EEPROM.commit();
}
...
unsigned int P_Memory::get_connection_time()
{
  ...
  ptr[3] = EEPROM.read(CONNECTION_TIME_ADDRESS + 3);
  return time;
}
void P_Memory::set_connection_time(unsigned int time)
{
  unsigned char* ptr=(unsigned char *)&time;
  EEPROM.write(CONNECTION_TIME_ADDRESS, ptr[0]);
  EEPROM.write(CONNECTION_TIME_ADDRESS + 1, ptr[1]);
  EEPROM.write(CONNECTION_TIME_ADDRESS + 2, ptr[2]);
  EEPROM.write(CONNECTION_TIME_ADDRESS+ 3, ptr[3]);
  EEPROM.commit();
}